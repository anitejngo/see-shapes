...
export const drawEverything = (canvasDiv: HTMLElement, canvas: any, shapes: Shape[]) => {
  try {
    ...
    if (ctx) {
      ...
      const zoomLevel = calculateZoomLevel(zoomPoints, canvas.width, canvas.height, PADDING);

      console.log(shapes, 'OP?');
      shapes.forEach((shape) => {
        drawShape(ctx, minX, minY, zoomLevel, PADDING, canvas, shape);
      });
    }
  } catch (er) {
    console.error('error while drawing', er);
  }
};
